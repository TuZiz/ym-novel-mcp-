...
姓名系统通过 `name_bank` 保存 `era`、`region`、`surnamePool`、`givenNamePool`、`bannedTokens`、`bannedFullNames`、`style`。更新已有姓名库时省略数组字段会保留旧池子，显式传空数组才会清空。默认禁用高 AI 味姓名 `叶辰`、`林枫`、`苏尘`、`萧凡`、`凌天`、`楚天`、`顾寒`、`秦渊`，以及 `辰`、`尘`、`天`、`凡`、`夜`、`渊`、`霆`、`宸`、`玄`、`帝` 等高频网文感字。`generate_character_name` 优先生成生活化姓名，`review_character_name` 返回 `aiScore`、`reason`、`suggestions`，`replace_character_name` 可传 `newName` 或自动生成可用名字，并会把旧名写入 aliases。
