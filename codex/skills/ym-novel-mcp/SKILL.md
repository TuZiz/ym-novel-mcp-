...
- If the user asks for next-chapter help, prefer `build_next_chapter_context` or the `write-next-chapter` prompt before inventing a manual outline.
...
4. When the user is ready to draft, call `build_next_chapter_context`.

### Ingest or Update a Chapter

1. Use `save_chapter` to persist finished chapter text.
2. Use `update_chapter_summary` if the summary needs tightening.
3. Add downstream state updates as needed:
   - `add_timeline_event`
   - `add_foreshadowing`
...
2. Run `build_next_chapter_context`.
3. If the user wants a direct writing prompt, use `write-next-chapter`.

## Tool Selection

- Project lifecycle: `create_project`, `get_project`, `list_projects`, `update_project`
