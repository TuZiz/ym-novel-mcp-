...
describe("search and context services", () => {
  it("searches chapters and builds next chapter context", () => {
    ...
    const matches = app.services.chapterService.searchChapters(
      project.id,
      "风铃",
      5,
    );
    ...
    expect(
      context.relevantWorldItems.some((item) => item.name === "白鹿书院"),
    ).toBe(true);
  ...
  it("bounds recent chapter content inside generated writing context", () => {
    const app = createTestApp();
    apps.push(app);
    ...
    const longContent =
      "开头线索必须保留。" + "中段铺陈。".repeat(1200) + "结尾钩子必须保留。";
    const chapter = app.services.chapterService.saveChapter({
      projectId: project.id,
      chapterIndex: 1,
      title: "长章",
    ...
    const fullChapter = app.services.chapterService.getChapter(
      project.id,
      chapter.id,
    );

    expect(fullChapter.content).toBe(longContent);
    expect(context.recentChapters[0]?.content.length).toBeLessThanOrEqual(2400);
    expect(context.recentChapters[0]?.content).toContain("开头线索必须保留");
    expect(context.recentChapters[0]?.content).toContain("结尾钩子必须保留");
    expect(context.recentChapters[0]?.content).toContain(
      "中间正文已为上下文省略",
    );
  });
});
